...
import { MessageCircle, Plus, Users, Send, Search, ArrowLeft, Hash, User as UserIcon } from 'lucide-react';
import { db } from '../firebase';
import Modal from '../components/Modal';
import LoadingScreen from '../components/LoadingScreen';
import { useAppData } from '../hooks/useAppData';
import { useAuth, useAuthorName } from '../hooks/useAuth';
import { ensureConversation, sendChatMessage, markChatRead, hasUnread, chatDisplayName } from '../utils/chat';
...
/**
 * Chat interno: mensajes directos y grupos entre los usuarios del app.
 * `chats` (miembros por email) + subcolección `messages`, todo en tiempo real.
 */
export default function ChatModule() {
  ...
  const [chats, setChats] = useState<ChatConversation[] | null>(null);
  ...
  // Conversaciones donde soy miembro, en vivo.
  useEffect(() => {
    if (!myEmail) { setChats([]); return; }
    ...
    const unsub = onSnapshot(q, snap => {
      ...
      setChats(rows);
    }, err => { console.error('Chat subscription failed', err); setChats([]); setError('Could not load conversations. Check the Firestore rules for the "chats" collection.'); });
    return () => unsub();
  }, [myEmail]);
  ...
  const handleSend = async (e: FormEvent) => {
    ...
    setIsSending(true);
    try {
      await sendChatMessage(activeChat.id, text, myEmail, authorName);
      setDraft('');
    } catch (err) {
  ...
  if (chats === null) return <LoadingScreen message="Loading chat..." />;
    ...
    <div className="card max-1400 chat-card catalog-manager-anim">
      <div className={`chat-layout${activeId ? ' has-active' : ''}`}>
        ...
        <aside className="chat-list">
          ...
          {error && <p className="alert error m-3">{error}</p>}
          <div className="chat-list-items">
            {filteredChats.length === 0 && <p className="chat-empty-hint">No conversations yet. Start one with <b>New</b>.</p>}
            {filteredChats.map(chat => {
              const unread = hasUnread(chat, myEmail);
              return (
                <button key={chat.id} type="button" className={`chat-item${chat.id === activeId ? ' active' : ''}${unread ? ' unread' : ''}`} onClick={() => setActiveId(chat.id)}>
                  ...
                  {unread && <span className="chat-unread-dot" aria-label="Unread messages" />}
                </button>
              );
            })}
          </div>
                    ...
                    <div key={m.id}>
                      ...
                      <div className={`chat-msg${mine ? ' mine' : ''}`}>
                        ...
                        <div className="chat-bubble">
                          {m.text}
                          <small className="chat-msg-time">{new Date(m.at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</small>
                        </div>
                      </div>
                    </div>
                  );
