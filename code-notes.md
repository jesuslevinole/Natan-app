...
## Ronda 11 (V0039) — fotos de artículos, chat interno y plantilla de importación

- **Fotos de artículos**: `utils/image.ts` (`fileToThumbDataUrl` redimensiona a 320px y comprime a JPEG si pasa de ~200KB; la foto se guarda como data URL dentro del doc del catálogo, por el límite de 1MB de Firestore — sin Storage). Tipo de campo nuevo `photo` en `FieldType`/catálogos: `item_names` suma el campo Photo; `CatalogsModule` lo renderiza con `PhotoInput` (subir/reemplazar/quitar con preview) en el form y `PhotoCell` (miniatura 42px, clic → lightbox) en la tabla. `ItemEntranceModule` arma `photoByItemName` (Map nombre→foto desde el catálogo) y muestra la miniatura junto a cada producto en las tablas de detalle (expandida y del form).
- **Chat interno** (`modules/ChatModule`, `utils/chat.ts`, colección `chats` + subcolección `messages`): DMs y grupos entre usuarios. DM con id determinístico (`dm_` + emails ordenados) para no duplicar conversaciones (setDoc merge); grupos con nombre + miembros. Suscripción en vivo (`array-contains` mi email), mensajes `orderBy at desc limit 80` revertidos, autoscroll, separadores por día, no leídos vía `lastReadBy` (emails con puntos→comas como clave) + `lastMessage`, marca de leído al abrir. Permiso `view_chat` (legacy `*`: todos lo ven por defecto). **Regla Firestore nueva**: `match /chats/{chatId} { allow read, write: if request.auth != null; match /messages/{msgId} { allow read, write: if request.auth != null; } }`.
- **Importación con plantilla**: `excel.ts` separa `Item Name` (columna nueva `item` en `ImportedInventoryRow`) del `Model #` (antes el modelo hacía de nombre); acepta hojas con cualquiera de los dos. La plantilla descargable ahora trae 2 hojas: `Products` (una fila por producto con Qty, 3 filas de ejemplo agrupadas por PO #) e `Instructions` (columna a columna qué es obligatorio). El import sigue creando item names y vendors faltantes; el dropzone explica el flujo "plantilla primero".
